...
5. Migration của gallery KHÔNG chạy trong PreSync job: worker chạy `migrate photo_gallery --database photos` lúc khởi động, nên database sự kiện và hook PreSync giữ nguyên như trước. Worker sẽ restart cho tới khi `photos-db` sẵn sàng.
