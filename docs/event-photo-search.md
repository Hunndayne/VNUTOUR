...
- Django app `photo_gallery`: album, ảnh, vector 128 chiều, lease xử lý và kết quả tìm kiếm có hạn 10 phút.
...
- YuNet + SFace FP32 qua OpenCV CPU. Model được kiểm tra SHA256 lúc build và lúc khởi tạo. Không cần GPU hoặc vector database riêng: dùng pgvector trong PostgreSQL hiện có, truy vấn cosine chính xác và gộp kết quả theo ảnh.
...
1. PostgreSQL phải cài pgvector **trên tất cả thành viên HA**, cùng phiên bản extension. Không tự đổi image database hoặc major version trên volume đang chạy. DBA tạo `CREATE EXTENSION IF NOT EXISTS vector;` trong DB ứng dụng trước khi migration nếu tài khoản migration không có quyền tạo extension.
...
1. Tạo Secret `photo-ai-secret` trong namespace đích, với `PHOTO_AI_TOKEN` ngẫu nhiên và `credentials.json` là key Drive. Dùng secret manager hoặc `kubectl create secret generic ... --from-file=PHOTO_AI_TOKEN=/secure/token --from-file=credentials.json=/secure/drive.json`. File mẫu trong component không được Kustomize apply.
...
4. Render bằng `kubectl kustomize k8s/kustomize/overlays/<environment>` và kiểm tra các giá trị trước khi sync. Component tự truyền bucket + token cho backend; API AI chỉ nhận token, worker chỉ mount key Drive.
5. PreSync migration job được đặt `PHOTO_GALLERY_ENABLED=1` trực tiếp để chạy migration ngay lần rollout đầu, kể cả ConfigMap cũ chưa được cập nhật. Các cấu hình DB/Secrets hiện có phải tồn tại trước PreSync.
...
- **Backup:** backup DB gồm vector, trạng thái và tombstone; backup R2 derivatives theo chính sách riêng. Ảnh gốc do Drive giữ. Sau restore, xác minh trạng thái album/tombstone trước khi mở lại public để tránh xuất bản lại ảnh đã gỡ sau mốc backup.
