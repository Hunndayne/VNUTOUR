...
4. Nếu môi trường có R2 thì dùng chung `R2_BUCKET`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_ENDPOINT_URL` của ứng dụng. Không cấu hình R2 (như staging) thì derivatives ghi xuống volume `media-data` và phục vụ ở `/media/event-photos/...`, giống submissions và ảnh feed: link không có chữ ký, chỉ dựa vào đoạn lease ngẫu nhiên trong đường dẫn, và không hết hạn. Prod nên dùng R2 để link có hạn 5 phút. Để `PHOTO_R2_BUCKET` trống để tự dùng bucket hiện có; không cần tạo bucket hoặc credentials riêng. Worker chỉ ghi preview/thumbnail dưới prefix `event-photos/` và cleanup theo object đã ghi nhận trong DB gallery, không quét/xóa toàn bucket. Nếu đã dùng bucket ảnh riêng, giữ override cũ để các key đã lưu vẫn trỏ đúng nơi.
...
- **Lập chỉ mục lại:** `POST /api/admin/photo-albums/<id>/reindex {}` (nút "Lập chỉ mục lại") xếp hàng lại MỌI ảnh, kể cả ảnh đã xong, nên áp dụng được ngưỡng mới mà không phải bump `MODEL_VERSION`. Ảnh được tải lại từ Drive và tạo preview mới; trong lúc xử lý ảnh tạm không hiện trong gallery.
- **Xóa album:** `DELETE /api/admin/photo-albums/<id>` xóa album, toàn bộ ảnh, vector và bản ghi preview trong database gallery. Thư mục/ảnh gốc trên Drive giữ nguyên; các object preview còn lại được worker dọn trong khoảng 20 phút. Khác với gỡ từng ảnh, ở đây không còn tombstone: tạo album mới từ cùng link Drive sẽ nhập lại từ đầu.
