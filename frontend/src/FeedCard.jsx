...
export default function FeedCard({ post, compact = false, onPostUpdated }) {
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      ...
      // A top-level comment also takes its replies with it (server cascades).
      const topLevel = comments.find((c) => c.id === commentId)
      const removed = topLevel ? 1 + (topLevel.replies?.length || 0) : 1
      let isReply = false
      setComments((prev) =>
        prev.map((c) => {
      ...
      const updated = {
        ...postState,
        comment_count: Math.max(0, (postState.comment_count || 0) - removed),
      }
      setPostState(updated)
      if (onPostUpdated) onPostUpdated(updated)
    } catch {
